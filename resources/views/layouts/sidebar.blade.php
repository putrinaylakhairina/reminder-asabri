<aside :class="{'translate-x-0': isSidebarOpen}" class="fixed top-0 left-0 z-40 w-64 h-screen pt-20 transition-transform -translate-x-full sm:translate-x-0 bg-primary border-r border-primary-dark shadow-xl" aria-label="Sidebar">
    <div class="h-full px-3 py-4 overflow-y-auto">
        <ul class="space-y-2 font-medium">
            {{-- Common Menu (Only for Admin) --}}
            @if(auth()->user()->role === 'admin')
                <li>
                    <a href="{{ route('dashboard') }}" class="flex items-center p-2 text-white rounded-lg hover:bg-primary-dark group {{ request()->routeIs('admin.dashboard') || request()->routeIs('dashboard') ? 'bg-primary-dark shadow-inner font-bold' : '' }}">
            ...
            @if(auth()->user()->role === 'asabri')
                <li>
                    <a href="{{ route('asabri.dashboard') }}" class="flex items-center p-2 text-white rounded-lg hover:bg-primary-dark group {{ request()->routeIs('asabri.dashboard') || request()->routeIs('dashboard') ? 'bg-primary-dark shadow-inner font-bold' : '' }}">
                        <x-lucide-file-text class="w-5 h-5 text-white opacity-80 group-hover:opacity-100" />
                        <span class="flex-1 ms-3 whitespace-nowrap">Dashboard Pribadi</span>
                    </a>
                </li>
            @endif
        </ul>
